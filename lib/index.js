...
exports.context = function (data, next) {
    ...
    // async handler
    var wait = 0;
    var done = function (err, inst) {
        --wait;

        // TODO are multiple error pushs possible?
        if (err) {
            return next(err);
        }
    ...
    data.app = App.cache(data.app);
    if (!data.app) {
        ++wait;
        App.factory(data.app, function (err, app) {
            data.app = app;
            done(err);
        });
    }
    ...
    if (data.comp) {
        data.comp = App.composition.cache(data.comp);
        if (!data.comp) {
            ++wait;
            App.composition.factory(data.comp, function (err, comp) {
                data.comp = comp;
                done(err);
            });
        }
